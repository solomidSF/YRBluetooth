...
/**
 *  Determines if given delegate should receive invocation immediately.
 *  If set to YES, all dispatch calls will be received immediately for all observers.
 *  It ignores provided queue for delegate, because invocation is done on the queue which initiated call.
 *  Default to NO.
 */
@property (nonatomic) BOOL syncedDispatch;
